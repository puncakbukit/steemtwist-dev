...
const LIVE_TWIST_TEMPLATES = [
  ...
  { id: "explorer", icon: "\ud83d\udd0d", name: "Data Explorer", desc: "Filterable and sortable local dataset",
    ...
    \"<th scope='col' aria-sort='\" + (sortBy===c ? (sortDir===1 ? \"ascending\" : \"descending\") : \"none\") + \"' style='padding:6px 8px;text-align:left;font-size:12px;'>\" +
    \"<button id='h_\" + c + \"' type='button' style='background:none;border:none;padding:0;margin:0;color:#a855f7;cursor:pointer;font-size:12px;text-align:left;' aria-label='Sort by \" + c + \"'>\" +
    c[0].toUpperCase() + c.slice(1) + (sortBy===c ? (sortDir===1?\" ↑\":\" ↓\") : \"\") + \"</button></th>\"
  ...
  methods: {
    ...
    buildSandboxDoc(userCode) {
      ...
      return "<!DOCTYPE html><html><head><meta charset='utf-8'>" + 
        ...
        "window.WebSocket=function(){throw new Error('Network blocked');};" +
        "window.open=()=>null;" +
        "var __stIntervals=[],__stTimeouts=[];var __si=window.setInterval.bind(window),__ci=window.clearInterval.bind(window),__st=window.setTimeout.bind(window),__ct=window.clearTimeout.bind(window);window.setInterval=function(fn,ms){var id=__si(fn,ms);__stIntervals.push(id);return id;};window.clearInterval=function(id){__stIntervals=__stIntervals.filter(function(x){return x!==id;});return __ci(id);};window.setTimeout=function(fn,ms){var id=__st(fn,ms);__stTimeouts.push(id);return id;};window.clearTimeout=function(id){__stTimeouts=__stTimeouts.filter(function(x){return x!==id;});return __ct(id);};function __clearTimers(){for(var i=0;i<__stIntervals.length;i++)__ci(__stIntervals[i]);for(var j=0;j<__stTimeouts.length;j++)__ct(__stTimeouts[j]);__stIntervals=[];__stTimeouts=[];}" +
        ...
        "var app={" +
        "render:function(h){__clearTimers();_root.innerHTML=sanitize(String(h));normalizeA11y();setTimeout(function(){var rh=document.body.scrollHeight;if(rh>40)parent.postMessage({type:'resize',height:rh+16},PARENT_ORIGIN);},0);}," +
        "text:function(s){__clearTimers();_root.textContent=String(s).slice(0,2000);normalizeA11y();}," +
